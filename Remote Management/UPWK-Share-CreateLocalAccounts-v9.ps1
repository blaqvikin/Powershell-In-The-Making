...
# Windows logon interface settings
...
# CRITICAL EDGE FIX: Force Edge to allow User-level (HKCU) GPO overrides on Entra/Intune joined systems
...
`$UserMap = @{
    ...
    'User_Russian' = 'ru-RU'; 'User_Japanese' = 'ja-JP'; 'User_Chinese' = 'zh-CN'; 'User_Vietnamese' = 'vi-VN'
}
...
if (`$UserMap.ContainsKey(`$Name)) {
    ...
    # Sets preferred web content results header (Accept-Language)
    Set-ItemProperty -Path `$EdgeUserPath -Name "DefinePreferredLanguages" -Value `$L
    # Forces Edge Application Menus/UI language layout to match
...
Write-Host "Please execute a complete system RESTART to apply language updates and confirm browser mapping."
