...
# Create main Company OU
Write-Host "Creating main Company OU..." -ForegroundColor Green
try {
    New-ADOrganizationalUnit -Name "Company" -Path $RootDN -ErrorAction Stop
    Write-Host "Created: OU=Company,$RootDN"
} catch {
    Write-Host "Company OU already exists or error: $_"
}

# Define OUs to create under Company (prefixed with ABC- to avoid name collisions on migration)
$OUs = @(
    "OU=ABC-Sales,OU=Company,$RootDN",
    "OU=ABC-Engineering,OU=Company,$RootDN",
    "OU=ABC-Finance,OU=Company,$RootDN",
    "OU=ABC-HR,OU=Company,$RootDN",
    "OU=ABC-Marketing,OU=Company,$RootDN",
    "OU=ABC-IT,OU=Company,$RootDN",
    "OU=ABC-Operations,OU=Company,$RootDN",
    "OU=ABC-Legal,OU=Company,$RootDN",
    "OU=ABC-Support,OU=Company,$RootDN",
    "OU=ABC-Disabled Users,OU=Company,$RootDN"
)

# Create OUs
Write-Host "Creating OUs under Company for $Domain..." -ForegroundColor Green
foreach ($OU in $OUs) {
    try {
...
# List of fictitious German names
$FictitiousNames = @(
    "Hans Müller",
    "Anna Schmidt",
    "Klaus Wagner",
...
foreach ($i in 0..19) {
    ...
    $UserPath = "OU=$OU,OU=Company,$RootDN"
...
foreach ($i in 1..7) {
    ...
    $Sam = "$UserBase`_ABC"
    $UserPrincipal = "$Sam@$Domain"
    $UserPath = "OU=ABC-Disabled Users,OU=Company,$RootDN"
...
foreach ($i in 1..20) {
    ...
    $ComputerPath = "OU=$($UserOUs[$OUIndex]),OU=Company,$RootDN"
